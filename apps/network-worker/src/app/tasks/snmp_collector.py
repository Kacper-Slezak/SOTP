...
async def _snmp_get_all_async(target):
    results = {}
    if not all([Config.SNMP_USER, Config.SNMP_AUTH, Config.SNMP_PRIV]):
        return results

    auth_data = snmp.UsmUserData(
        Config.SNMP_USER,
        authKey=Config.SNMP_AUTH,
        privKey=Config.SNMP_PRIV,
        authProtocol=snmp.usmHMACSHAAuthProtocol,
        privProtocol=snmp.usmAesCfb128Protocol,
    )

    var_objs = [snmp.ObjectType(snmp.ObjectIdentity(oid)) for oid in OIDS.values()]

    engine = snmp.SnmpEngine()
    error_indication, error_status, _, var_binds = await snmp.get_cmd(
        engine,
        auth_data,
        await snmp.UdpTransportTarget.create((target, 161), timeout=1, retries=1),
        snmp.ContextData(),
        *var_objs,
    )
...
def snmp_get_all(target):
    return asyncio.run(_snmp_get_all_async(target))
