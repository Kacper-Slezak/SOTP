...
from sqlalchemy import create_engine # Use create_engine for sync connection in migrations
...
def get_target_metadata():
    ...
    if db_name == 'timescaledb':
        ...
        print("Defaulting to PostgreSQL metadata (use -x db=postgres or -x db=timescaledb)")
        return postgres_metadata
...
def get_db_url():
    db_name = context.get_x_argument(as_dictionary=True).get('db')
    if db_name == 'timescaledb':
        print(f"Using TimescaleDB URL: {Config.TIMESCALEDB_URL.replace('+asyncpg', '+psycopg2')}")
        ...
        print(f"Using PostgreSQL URL: {Config.POSTGRES_URL.replace('+asyncpg', '+psycopg2')}")
